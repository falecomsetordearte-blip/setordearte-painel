import { v4 as uuidv4 } from 'uuid'; // Ferramenta para gerar tokens únicos
import axios from 'axios'; // Ferramenta para fazer chamadas de API
import bcrypt from 'bcryptjs'; // Ferramenta para criptografar senhas
import nodemailer from 'nodemailer'; // Ferramenta para enviar e-mails
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // ETAPA 2: GERAR TOKEN DE SESSÃO E HASH DA SENHA
        ...
        const createCompanyResponse = await axios.post(`${BITRIX24_API_URL}crm.company.add.json`, {
            fields: {
                ...
                'UF_CRM_COMPANY_1656518412966': cnpj
            }
        });
        ...
        // ETAPA 6: GERAR LINK DE PAGAMENTO DA ASSINATURA NO ASAAS
        ...
        // ETAPA FINAL: RETORNAR O LINK DE CHECKOUT PARA O FRONT-END
        ...
        // Verifica se o erro é de envio de e-mail (geralmente por e-mail inválido)
        if (error.responseCode === 550 || (error.command && error.command.includes('RCPT TO'))) {
             return res.status(400).json({ status: "error", message: "E-mail inválido. Por favor, confira o e-mail utilizado ou use um e-mail existente." });
        }
        
        // Erro genérico para todas as outras falhas
        res.status(500).json({ message: 'Ocorreu um erro interno durante o cadastro.' });
    }
}
